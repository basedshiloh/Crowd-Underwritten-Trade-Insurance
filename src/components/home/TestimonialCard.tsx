...
const TestimonialCard = ({ name, image, quote, twitterUrl }: TestimonialCardProps) => {
  return (
    <div className="border-stroke-1 mt-6 max-w-[500px] rounded-[20px] border bg-white p-6 sm:p-8">
      ...
      <figure className="flex items-center gap-3">
        <Image
          src={image}
          className="size-11 rounded-full bg-linear-[156deg,#F4F8E7_12.92%,#D485FF_91%] object-cover object-center"
          alt={`${name}'s avatar`}
          width={44}
          height={44}
          loading="lazy"
        />
        <h3 className="text-tagline-1 leading-[1.5] font-semibold sm:text-lg">{name}</h3>
      </figure>
    </div>
  );
};
