import type { CoverageRequest, UnderwriterStake } from "./coverage";
...
export function createCoverageRequest(input: CreateCoverageInput): { ok: true; request: CoverageRequest } | { ok: false; error: string } {
  const now = new Date();
