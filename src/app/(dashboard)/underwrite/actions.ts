...
export async function underwriteAction(
  _prev: UnderwriteState | null,
  formData: FormData
): Promise<UnderwriteState> {
  ...
  if (!allowed) {
    return {
      ok: false,
      error: 'Your token hold % is below the tier required to underwrite this coverage size.',
    };
  }
